...
# use double quotes for pasting in multi line
ai () {
...
alias gl='git log'
alias gll='git log --oneline --pretty=custom'
...
alias renix='nix profile add "path:$NIX_HOME" && echo "profile added" && nix flake update --flake "path:$NIX_HOME" && echo "flake updated" && nix profile upgrade --all && echo "profile upgraded"'
